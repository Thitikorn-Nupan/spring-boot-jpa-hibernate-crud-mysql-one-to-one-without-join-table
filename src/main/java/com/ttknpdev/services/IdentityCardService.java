...
public class IdentityCardService {
    ...
    public IdentityCard retrieveIdentityCard(String cidNumber) {
        // for test my exception if cidNumber is not found it will throw my exception
        return identityCardRepository
                .findById(cidNumber)
                .orElseThrow(() -> new ContentNotAllowed("There is no cid number " + cidNumber + "."));
    }

    public Boolean addIdentityCard(IdentityCard identityCard) {
        return identityCardRepository.save(identityCard).getIdentity() != null;
        /**
    ...
    public Boolean removeIdentityCard(String cidNumber) {
        return identityCardRepository.findById(cidNumber)
                .map(identityCard -> {
    ...
    public Boolean editIdentityCard(String cidNumber, IdentityCard identityCard) {
        return identityCardRepository.findById(cidNumber)
                .map(identityCardFound -> {
                    ...
                    identityCardFound.setBorn(identityCard.getBorn());
